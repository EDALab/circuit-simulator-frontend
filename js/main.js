...
const grid = (function SchematicsGrid() {
  ...
  self.bias = function (x, y) {
    if (x === u && y === u) {
      ...
      // TODO: what case does this correspond to?
      placeX = x[0];
      placeY = x[1];
    }
  };
  ...
  self.copy = function (arr) {
    ...
    for (let i = 0; i < data.length; i++) {
      if (data[i].current.status === "move") {
        // TODO: what is this current property of the parts collection?
        move.push(data[i]);
      }
    }
    move = save(move);
...
function mousemoveEvent(event) {
  ...
  switch (true) {
    ...
    case grid.moveParts: {
      console.log(
        "This is triggered when ur mouse is moving a part and dragging it around"
      );
...
function contextSet(event, status) {
  ...
  if (status.indexOf("part") !== -1) {
    // this says: find the index of the substring 'part' in the status string, and if it
    ...
    for (let i = 0; i < 4; i++) {
      // TODO: why is the number strictly 4?
...
function createGraph(data) {
  const ratio = 1.6, //Page aspect ratio
    ...
    maxForm =
      !!data.voltage.length + //How many waveform interfaces are needed
      !!data.current.length,
    ...
    graphWidth =
      totalHeight * ratio < totalWidth ? totalHeight * ratio : totalWidth,
  ...
  for (let i = 0; i < 2; i++) {
    ...
    if (data[label].length) {
      ...
      graphForm
        .get(sub)
        .prop("_data", new Graph(data[label], graphForm[sub], label));
    }
  }
  ...
  const height = graphPage.height(),
    ...
    actionBottom = 63.6,
    R = Math.sqrt(
      (actionRight - width) * (actionRight - width) +
        (actionBottom - height) * (actionBottom - height)
    ),
    clipPath =
      [
  ...
  const circle = graphPage.append(
    $("<div>", {
      id: "background-circle",
      style:
        "position: absolute; background-color: #66CCCC; border-radius: 50%;" +
...
action.on("click", "#fab-run", function (event) {
  ...
  if (temp_var.length == 0) {
    ...
    // if temp_var array is longer than 1, then this means there is more than 1 circuit on the grid
    grid.error(
