...
function joinNode(pin, nodeIndex) {
  if (!nodeList[nodeIndex].includes(pin)) {
    // if the node does not include this pin name, we add the pin
...
function nodeId(input) {
  ...
  for (var device in input) {
    ...
    if (component.type == "REF") {
      ...
      for (
        var connLineIndex = 0;
        connLineIndex < connLines.length;
        connLineIndex++
      ) {
      ...
      // same procedure as above with ground pin
      // Wire
      ...
      for (
        var connLineIndex = 0;
        connLineIndex < connLines0.length;
        connLineIndex++
      ) {
      ...
      for (
        var connLineIndex = 0;
        connLineIndex < connLines1.length;
        connLineIndex++
      ) {
  ...
  for (var components in output) {
    ...
    for (
      var component = 0;
      component < output[components].length;
      component++
    ) {
